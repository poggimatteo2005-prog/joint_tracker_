-- Extends admin_dashboard_stats() with three new keys for the /admin page:
--   recent_sessions   - last 20 smokes rows ordered by the LOGGED datetime (date + time),
--                        every column via to_jsonb(s.*) plus username/email joined in.
--   session_locations - every geolocated smokes row (latitude/longitude not null) for the map.
--   session_users     - distinct {user_id, username, email} among geolocated rows (filter dropdown).
-- Still SECURITY DEFINER + hardcoded owner-email gate; no new tables, no new RLS.
-- NOT auto-applied on push: run in the Supabase SQL editor (or via MCP) manually.
...
begin
	...
	recent_sessions as (
		select jsonb_agg(
			(to_jsonb(l) - 'logged_at') || jsonb_build_object(
	...
	session_users as (
		...
		from (
			select distinct s.user_id, p.username, u.email
			from public.smokes s
