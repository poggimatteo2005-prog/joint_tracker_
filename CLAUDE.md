...
- **Avatar utente — leaderboard/modal amico**: le 4 RPC `get_global_leaderboard`, `get_friends_leaderboard`, `get_friends_shared_leaderboard`, `get_friend_stats` ritornano ora `avatar_url` direttamente (migration `20260830150000_leaderboard_rpcs_avatar_url.sql`); il client legge quella colonna e `fetchAvatarMap()` è stato rimosso. Guest mode: `jt_guest_avatar` in `localStorage` usa solo i preset, migrato a login via `migrateGuestAvatar()`.
